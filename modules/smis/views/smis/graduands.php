...
// Push the data into the array   
     $res = $data;
		foreach($res as $k){	
			$col_name=$k["DEGREE_TYPE"];
			$total_amount=$k["GRADUANDS"];
...
/*Create an object for the column chart using the FusionCharts PHP class constructor. Syntax for the constructor is ` FusionCharts("type of chart", "unique chart id", width of the chart, height of the chart, "div id to render the chart", "data format", "data source")`. Because we are using JSON data to render the chart, the data format will be `json`. The variable `$jsonEncodeData` holds all the JSON data for the chart, and will be passed as the value for the data source parameter of the constructor.*/
        ...
        $columnChart = new app\modules\smis\fusion_charts\src\FusionCharts("line", "myFirstChart" , '50%', 500, "chart-1", "json", $jsonEncodedData);
...
echo Select2::widget([
    'name' => 'ac_year',
    'data' => $acyear_arr,
    'options' => ['placeholder' => '--Select Graduation Date--','onchange'=>'this.form.submit()'],
    'pluginOptions' => [
        'allowClear' => true
    ],
]);
...
<div class = "panel panel-info">
   <div class = "panel-heading">
      <h3 class = "panel-title" align ="center" ><?php echo "TOTAL GRADUANDS PER DEGREE TYPE ". $gtitle?></h3>
   </div>
<style>
    ...
    <tr>
	  ...
	  <th scope="col">GRADUANDS</th>
		...
		foreach($res as $k){
			...
			<tr>
			<td><?= $counter;?></td>
				<td><a href="#"a><?= $k["DEGREE_TYPE"] ;?></td>
				<td><?= number_format($k["GRADUANDS"]); ?></td>
			...
			<?php
				$counter++;
				 $total_sum+=$k["GRADUANDS"];
		}
		?>	
			</tr>
			<tr>
			<td></td><td><strong class="pull-right" >TOTAL GRADUANDS</strong></td><td><strong><?= number_format($total_sum) ;?></strong></td>
			</tr>
