...
// yii\helpers\Url
   ...
   // $result = $dbhandle->query($strQuery) or exit("Error code ({$dbhandle->errno}): {$dbhandle->error}");
    ...
    // If the query returns a valid response, prepare the JSON string
        ...
        $arrData = array(
            "chart" => array(
              "caption" => "VEHICLE REPAIR COSTS $gtitle",
...
if($range) {
    ...
    echo Html::dropDownList('yr', null,
        ArrayHelper::map(TRANSMIS::REPAIR_YEARLIST(), 'yr', 'yr'),
...
<h4 class="text-center"><?php echo "VEHICLE REPAIR COSTS ".$gtitle;?></h4>
<p>
<style>
.xpTable th,.xpTable td{ padding:5px; background-color:#fff; border:#ccc solid 1px;}
	...
	<table  align= "center" class="xpTable" border=1 class="table table-bordered table-condensed" style="line-height: 24px; width:70%;">
		...
		foreach($res as $k){
			...
			<td><?= $counter;?></td>
				<td><?= $k["MONTH"] ;?></td>
				<td><?= number_format($k["INVOICE_AMOUNT"]); ?></td>
