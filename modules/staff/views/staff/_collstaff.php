	...
	<table class="table table-bordered table-hover table-striped table-nonfluid">
		...
		foreach($res as $k){
			...
			<tr style="font-weight: bold;">
				...
				<td nowrap class="text-right"> <?= $k["TS"]?'<a href="'.Url::to(['', 'C' => $k["COL_CODE"], 'T' => 'T']).'">'.number_format($k["TS"]).'</a>':'<span style="color:red;">0</span>' ?></td>
				<td nowrap class="text-right"> <?= $k["NTS"]?'<a href="'.Url::to(['', 'C' => $k["COL_CODE"]]).'">'.number_format($k["NTS"]).'</a>':'<span style="color:red;">0</span>' ?></td>
				<td nowrap class="text-right"> <?= $k["P"]?'<a href="'.Url::to(['', 'C' => $k["COL_CODE"], 'T' => 'P']).'">'.number_format($k["P"]).'</a>':'<span style="color:red;">0</span>' ?></td>
			</tr>
			<?php
		}
		?>
		...
		<tfoot>
			<tr>
				...
				<th class="text-right"><?=number_format($sumT)?></th>
				<th class="text-right"><?=number_format($sumNT)?></th>
				<th class="text-right"><?=number_format($sumP)?></th>
			</tr>
			<tr>
				<th class="text-primary" colspan=3>GRAND TOTAL</th>
				<th class="text-right"><?=number_format(($sumNT+$sumT+$sumP))?></th>
			</tr>
		</tfoot>
	</table>
	</div>
