...
// yii\helpers\Url
?>
<h4 class="text-center">Personnel Appraisal Status. Appraisal Period : 2015/2016</h4>
<p>
	<table  align= "center" class="xpTable" border=1 class="table table-bordered table-condensed">
		...
		foreach($res as $k){
			...
			<tr style="font-weight: bold;">
				<td nowrap><?= $k["COL_CODE"] ?></td>
				<td nowrap><?= $k["COL_NAME"] ?></td>
				<td nowrap><?= $k["TOTAL"] ?></td>
				<td nowrap><?= $k["APPRAISED"] ?></td>
				<td nowrap><?= number_format(($k["APPRAISED"]/$k["TOTAL"])*100,2) ?></td>
			</tr>
			<?php
		}
		?>
   ...
   // $result = $dbhandle->query($strQuery) or exit("Error code ({$dbhandle->errno}): {$dbhandle->error}");
    ...
    // If the query returns a valid response, prepare the JSON string
        ...
        $arrData = array(
            "chart" => array(
              "caption" => "% of College Staff with complete Supervisor Evaluation for the year 2015/2016",
...
/*Create an object for the column chart using the FusionCharts PHP class constructor. Syntax for the constructor is ` FusionCharts("type of chart", "unique chart id", width of the chart, height of the chart, "div id to render the chart", "data format", "data source")`. Because we are using JSON data to render the chart, the data format will be `json`. The variable `$jsonEncodeData` holds all the JSON data for the chart, and will be passed as the value for the data source parameter of the constructor.*/
        ...
        $columnChart = new app\modules\uspas\fusion_charts\src\FusionCharts("column3d", "myFirstChart" , 800, 500, "chart-1", "json", $jsonEncodedData);
