...
// yii\helpers\Url
   ...
   // $result = $dbhandle->query($strQuery) or exit("Error code ({$dbhandle->errno}): {$dbhandle->error}");
    ...
    // If the query returns a valid response, prepare the JSON string
        ...
        $arrData = array(
            "chart" => array(
              "caption" => "% of {$user['COL_NAME']} Staff with complete Supervisor Evaluation for the year $yr",
             ...
             // "paletteColors" => "#2bb9cf",
			  ...
			  "yaxismaxvalue"=>  "100",
			   ...
			   "exportFileName"=>"Appraisal_status_percollegeforyear_$yr",
        ...
        $arrData['data']=array(
			...
			array(
					"label" => 'Percentage of Staff Appraised',
					 "value" => number_format(($k["SUPERVISOR_EVALUATED"]/$k["TOTAL"])*100,2),
					 "issliced"=> "1"
					
				),
				array(
					"label" => 'Percentage of Staff not Appraised',
					 "value" => number_format((100-($k["SUPERVISOR_EVALUATED"]/$k["TOTAL"])*100),2),
					  "issliced"=> "1"
				)
...
<h4 class="text-center">Appraisal Status Per Department in <?= $user['COL_NAME'];?>. Appraisal Period :<?php echo $yr; ?></h4>
<p>
<style>
	...
	<table  align= "center" class="xpTable" border=1 class="table table-bordered table-condensed" style="line-height: 24px; width:70%;">
		...
		foreach($res as $k){
			...
			<tr>
				...
				<td><?= $k["SUPERVISOR_EVALUATED"] ?></td>
				<td><?= $k["TOTAL"] ?></td>			
				
				<td><?= number_format(($k["SUPERVISOR_EVALUATED"]/$k["TOTAL"])*100,2) ?></td>
			</tr>
			<?php
		}
		?>
